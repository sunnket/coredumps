# -*- coding: utf-8 -*-
"""Batch E - Dynamic Programming, Bit Manipulation, Math & Greedy."""
...
P(
    ...
    constraints=["1 <= len(nums) <= 2500", "-10^4 <= nums[i] <= 10^4", "Follow-up: can you do O(n log n)?"],
    hints=[
...
P(
    ...
    constraints=["1 <= len(nums) <= 3 * 10^4", "Every element appears twice except one", "O(1) extra space"],
    hints=[
...
P(
    ...
    examples=[
        ("A board where row 0 is ['5','3','.','.','7','.','.','.','.']", "True", "No conflicts among the filled cells."),
        ("The same board with the top-left 5 changed to 8", "False", "Two 8s in the top-left box."),
    ],
    constraints=["board is exactly 9 x 9", "Each cell is a digit 1-9 or '.'", "The board need not be solvable"],
    hints=[
...
P(
    ...
    constraints=["1 <= len(nums) <= 10^5", "-2^31 <= nums[i] <= 2^31 - 1", "O(1) auxiliary space required"],
    hints=[
